...
with open("annotations.txt", "r") as a_file:
    for line in a_file:
        #strip each line, remove name of image and then create an np array to store information
        stripped_line = line.strip()
        stripped_list = np.array(stripped_line.lstrip(img_list[i]).split())
        #Append zeros to end if size is too small (need size 9 array)
...
#Iterate through all images, and draw rectangeles based off of annotations.txt
i = 0
for name in img_list:
    img = cv.imread("images/" + name)
    #Check if we draw one or two rectangles, then draw them
